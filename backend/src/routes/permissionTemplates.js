    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,15,40,0.4)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ background: '#fff', borderRadius: 14, width: 560, maxHeight: '85vh', overflowY: 'auto', padding: 24 }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h3 style={{ fontSize: 19, fontWeight: 700, color: '#1f1f3d', margin: 0 }}>{isEdit ? 'Edit Permission Template' : 'Add Permission Template'}</h3>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 18 }}>
          <div style={{ flex: 1 }}>
            ...
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Telecaller Permissions" style={{ width: '100%', marginTop: 6, padding: '9px 12px', border: '1px solid #e0ddf0', borderRadius: 8, fontSize: 13.5, boxSizing: 'border-box' }} />
          </div>
          <div style={{ width: 160 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
          <div>
            <h4 style={{ fontSize: 13.5, fontWeight: 700, color: '#1f1f3d', margin: '0 0 6px' }}>Leads</h4>
            <label style={{ fontSize: 12.5, color: '#666' }}>View scope</label>
            <select value={form.permissions.leads.view} onChange={e => setPerm('leads', 'view', e.target.value)} style={{ width: '100%', marginTop: 4, marginBottom: 6, padding: '6px 8px', border: '1px solid #e0ddf0', borderRadius: 6, fontSize: 12.5 }}>
          ...
          <div>
            <h4 style={{ fontSize: 13.5, fontWeight: 700, color: '#1f1f3d', margin: '0 0 6px' }}>Campaigns</h4>
            <Bool group="campaigns" k="view" label="View" />
            <Bool group="campaigns" k="manage" label="Manage" />

            <h4 style={{ fontSize: 13.5, fontWeight: 700, color: '#1f1f3d', margin: '14px 0 6px' }}>Reports</h4>
            <Bool group="reports" k="view" label="View" />

            <h4 style={{ fontSize: 13.5, fontWeight: 700, color: '#1f1f3d', margin: '14px 0 6px' }}>Users</h4>
            <Bool group="users" k="view" label="View" />
            <Bool group="users" k="manage" label="Manage" />

            <h4 style={{ fontSize: 13.5, fontWeight: 700, color: '#1f1f3d', margin: '14px 0 6px' }}>Settings & Workflows</h4>
    ...
    <div style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: '#1f1f3d', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1f1f3d" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
          Permission Templates
        </h2>
        <button onClick={() => { setEditTarget(null); setModalOpen(true); }} style={{ padding: '9px 18px', borderRadius: 8, border: 'none', background: '#7c5cf0', color: '#fff', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ display: 'flex', gap: 24, borderBottom: '1px solid #eee', marginBottom: 14 }}>
        {['all', 'defaults'].map(t => (
          <span key={t} onClick={() => setTab(t)} style={{ padding: '8px 2px', fontSize: 14, fontWeight: 600, cursor: 'pointer', color: tab === t ? '#7c5cf0' : '#888', borderBottom: tab === t ? '2px solid #7c5cf0' : '2px solid transparent', textTransform: 'capitalize' }}>
            {t}
          </span>
        ))}
      </div>

      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." style={{ width: '100%', padding: '10px 14px', border: '1px solid #e0ddf0', borderRadius: 8, fontSize: 13.5, marginBottom: 8, boxSizing: 'border-box' }} />
      <p style={{ fontSize: 12.5, color: '#999', margin: '0 0 14px' }}>{filtered.length} templates found</p>

      <div style={{ background: '#fff', border: '1px solid #eee', borderRadius: 10, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', padding: '12px 18px', fontSize: 12.5, fontWeight: 700, color: '#666', background: '#fafafd' }}>
        ...
        {loading ? (
          ...
          filtered.map(t => (
            <div key={t._id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', padding: '12px 18px', fontSize: 13.5, borderTop: '1px solid #f4f3fa', alignItems: 'center' }}>
              <span style={{ fontWeight: 600, color: '#333', display: 'flex', alignItems: 'center', gap: 6 }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#7c5cf0" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                {t.name}
              </span>
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                {t.isDefault && t.assignedCount > 0 ? (
                  <span style={{ cursor: 'pointer', color: '#7c5cf0' }} title="View">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                  </span>
                ) : (
                  <span onClick={() => { setEditTarget(t); setModalOpen(true); }} style={{ cursor: 'pointer', color: '#7c5cf0' }} title="Edit">
