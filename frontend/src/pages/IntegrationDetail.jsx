  ...
  return (
    <div style={{ padding: '20px 28px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <button onClick={() => navigate('/integrations')} style={{ background: 'none', border: 'none', color: 'var(--theme-primary-alt)', cursor: 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', gap: 4, padding: 0 }}>
        ...
        <div>
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', marginBottom: 28 }}>
            {[
              ...
              <div key={stat.label} style={{ background: '#fff', border: '1px solid var(--theme-border-tint)', borderRadius: 12, padding: '16px 24px', minWidth: 160 }}>
          ...
          {/* Webhook URL */}
          <div style={{ background: '#fff', border: '1px solid var(--theme-border-tint)', borderRadius: 12, padding: 20, marginBottom: 20 }}>
            <h4 style={{ margin: '0 0 12px', fontSize: 15, color: 'var(--theme-text-strongest)' }}>Webhook URL</h4>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => { navigator.clipboard.writeText(webhookUrl); alert('Copied!'); }}
                style={{ padding: '8px 14px', borderRadius: 8, border: '1.5px solid var(--theme-primary-alt)', background: 'var(--theme-primary-alt)', color: '#fff', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
                Copy
              </button>
            </div>
            <div style={{ marginTop: 12, background: 'var(--theme-surface-faint2)', borderRadius: 8, padding: 14 }}>
      ...
      {activeTab === 'configuration' && (
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: 32, overflowX: 'auto', paddingBottom: 4 }}>
            {STEPS.map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center' }}>
                ...
                {i < STEPS.length - 1 && (
                  <div style={{ height: 1, width: 60, background: i < step ? 'var(--theme-primary-alt)' : '#d1d5db', margin: '0 4px', marginBottom: 24 }} />
                )}
              </div>
            ))}
          </div>
          ...
          {/* Step content */}
          <div style={{ background: '#fff', border: '1px solid var(--theme-border-tint)', borderRadius: 12, padding: 28 }}>
            {step === 0 && (
              <div>
                <h4 style={{ margin: '0 0 20px', color: 'var(--theme-text-strongest)' }}>Integration Details</h4>
                  ...
                  {['apiKey', 'accessToken', 'pageId', 'formId'].map(field => (
                    <div key={field}>
                      ...
                      <input
                        ...
                        style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--theme-border-tint)', borderRadius: 8, fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
                      />
                    </div>
                  ))}
                </div>
            ...
            {step === 1 && (
              <div>
                <h4 style={{ margin: '0 0 8px', color: 'var(--theme-text-strongest)' }}>Map Integration Fields</h4>
                ...
                <div style={{ display: 'grid', gap: 12 }}>
                  {['name', 'phone', 'email', 'location'].map(field => (
                    <div key={field} style={{ display: 'grid', gridTemplateColumns: '120px 20px 1fr', alignItems: 'center', gap: 12 }}>
                      <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--theme-text-strongest)', textTransform: 'capitalize' }}>{field}</span>
                      <span style={{ color: '#9ca3af' }}>←</span>
                      <input
                        value={fieldMapping[field] || ''}
                        onChange={e => setFieldMapping(prev => ({ ...prev, [field]: e.target.value }))}
                        placeholder={`Source field for "${field}"`}
                        style={{ padding: '8px 12px', border: '1px solid var(--theme-border-tint)', borderRadius: 8, fontSize: 14, outline: 'none' }}
                      />
                    </div>
                  ))}
                </div>
            ...
            {step === 2 && (
              <div>
                <h4 style={{ margin: '0 0 8px', color: 'var(--theme-text-strongest)' }}>Choose Campaign</h4>
                ...
                <select
                  ...
                  style={{ width: '100%', padding: '10px 12px', border: '1px solid var(--theme-border-tint)', borderRadius: 8, fontSize: 14, outline: 'none', background: '#fff' }}
                >
            ...
            {step === 3 && (
              <div>
                <h4 style={{ margin: '0 0 8px', color: 'var(--theme-text-strongest)' }}>Lead Distribution</h4>
            ...
            {step === 4 && (
              <div>
                <h4 style={{ margin: '0 0 8px', color: 'var(--theme-text-strongest)' }}>Webhook Setup</h4>
                ...
                <div style={{ background: 'var(--theme-surface-faint2)', borderRadius: 8, padding: 16, border: '1px solid var(--theme-border-tint)', marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(webhookUrl); alert('Copied!'); }}
                      style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid var(--theme-primary-alt)', background: 'var(--theme-primary-alt)', color: '#fff', cursor: 'pointer', fontSize: 12, whiteSpace: 'nowrap' }}>
                      Copy
                    </button>
                  </div>
                </div>
                <div style={{ background: 'var(--theme-surface-faint2)', borderRadius: 8, padding: 16, border: '1px solid var(--theme-border-tint)' }}>
            ...
            {step === 5 && (
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <button onClick={() => setActiveTab('leads')} style={{ marginTop: 16, padding: '10px 24px', borderRadius: 8, border: 'none', background: 'var(--theme-primary-alt)', color: '#fff', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
                  View Leads
                </button>
              </div>
            )}
          ...
          {step < 5 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 20 }}>
              <button
                ...
                disabled={step === 0}
                style={{ padding: '9px 24px', borderRadius: 8, border: '1.5px solid var(--theme-border-tint)', background: '#fff', color: '#6b7280', fontWeight: 600, fontSize: 14, cursor: step === 0 ? 'default' : 'pointer', opacity: step === 0 ? 0.5 : 1 }}
              >
                Back
              </button>
              <button
                ...
                style={{ padding: '9px 24px', borderRadius: 8, border: 'none', background: 'var(--theme-primary-alt)', color: '#fff', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}
              >
      ...
      {activeTab === 'leads' && (
        <div>
          ...
          {leads.length === 0 ? (
            <div style={{ background: '#fff', border: '1px solid var(--theme-border-tint)', borderRadius: 12, padding: 40, textAlign: 'center', color: '#9ca3af' }}>
